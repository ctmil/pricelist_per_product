...
class product_pricelist_item(osv.osv):
    ...
    _columns = {
        'customer_ids': fields.many2many('res.partner', id1='pricelist_item_id', id2='partner_id', string='Customers'),
	    }
...
class product_pricelist(osv.osv):
	...
	customer_ids = {}
        ...
        for p in products:
            categ = p.categ_id
            customer_ids[p.customer_id.id] = True
            while categ:
        ...
        begin_sql_stmt = 'SELECT i.id FROM product_pricelist_item AS i \
                        INNER JOIN product_pricelist_version AS v on i.price_version_id = v.id \
                        INNER JOIN product_pricelist AS p on p.id = v.pricelist_id \
                        WHERE p.active = true and v.active = true \
                        AND p.id = %s AND v.id = %s \
                        AND (product_tmpl_id IS NULL OR product_tmpl_id = any(array%s)) \
                        AND (product_id IS NULL OR (product_id = any(array%s))) \
                        AND ((categ_id IS NULL) OR (categ_id = any(array%s))) '%(pricelist_id,version_id,prod_tmpl_ids,prod_ids,categ_ids)
        end_sql_stmt = 'ORDER BY sequence, min_quantity desc'
        if customer_ids and customer_ids[0]:
                begin_sql_stmt = begin_sql_stmt + 'AND ((customer_id IS NULL) OR (customer_id = any(array%s))) '%(customer_ids)
        sql_statement = begin_sql_stmt + end_sql_stmt
        ...
        for product, qty, partner in products_by_qty_by_partner:
            ...
            for rule in items:
                ...
                if rule.customer_ids:
                    not_found = True
                    for customer_id in rule.customer_ids.ids:
